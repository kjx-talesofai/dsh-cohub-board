...
// ---- CSS (injected once; same mechanism shipped client plugins use) ----
...
const TOKEN_TO_HEX = { brand: "#e8450e", rose: "#e11d48", amber: "#d97706", green: "#16a34a", blue: "#2563eb", violet: "#7c3aed", black: "#000000", neutral: "#5f6368", white: "#ffffff" };
const HEX_TO_TOKEN = { "#e8450e": "brand", "#e11d48": "rose", "#d97706": "amber", "#16a34a": "green", "#2563eb": "blue", "#7c3aed": "violet", "#000000": "black" };
const TOOLS = [
...
function bboxFrame(pts, pad) {
	...
	for (let i = 0; i < pts.length; i++) { minX = Math.min(minX, pts[i].x); minY = Math.min(minY, pts[i].y); maxX = Math.max(maxX, pts[i].x); maxY = Math.max(maxY, pts[i].y); }
...
function localBuildItem(hostKind, colorHex, size, points, text, id) {
	...
	const pts = points || [];
	if (hostKind === "line" && pts.length >= 2) { const a = pts[0], b = pts[1]; const pad = Math.max(8, size * 2); return { id, type: "arrow", frame: bboxFrame([a, b], pad), start: { x: a.x, y: a.y }, end: { x: b.x, y: b.y }, bend: 0, color: token, size, arrowStart: false, arrowEnd: false, label: "" }; }
	if (hostKind === "rect" && pts.length >= 2) return { id, type: "geo", frame: bboxFrame(pts, 0), geo: "rectangle", text: "", color: token, fillOpacity: 0 };
	if (hostKind === "ellipse" && pts.length >= 2) return { id, type: "geo", frame: bboxFrame(pts, 0), geo: "ellipse", text: "", color: token, fillOpacity: 0 };
	if (hostKind === "frame" && pts.length >= 2) return { id, type: "frame", frame: bboxFrame(pts, 0), label: "", color: token };
	if (hostKind === "text") { const p = pts[0] || { x: 0, y: 0 }; return { id, type: "text", frame: { x: p.x, y: p.y, width: 120, height: 64, rotation: 0 }, text: text || "A", color: token, fontSize: Math.max(2, Math.min(512, size * 8)) }; }
	const pad2 = Math.max(8, size * 2);
	const frame = bboxFrame(pts, pad2);
	return { id, type: "draw", frame, points: pts.map((q) => ({ x: q.x - frame.x, y: q.y - frame.y, p: 0.5 })), color: token, size };
}

export const inject = ["slots", "remote", "remote.board", "timer"];
...
export function apply(ctx) {
	...
	function drawItem(c, item) {
		const f = item.frame || { x: 0, y: 0, width: 1, height: 1, rotation: 0 };
	...
	function drawPreview(c) {
		...
		if (tool === "draw" || tool === "arrow") {
			if (curPoints.length >= 2) { c.beginPath(); c.moveTo(a.x, a.y); for (let i = 1; i < curPoints.length; i++) c.lineTo(curPoints[i].x, curPoints[i].y); c.stroke(); }
	...
	function redraw() {
		...
		if (gridOn) {
			...
			c.beginPath();
			for (let gx = Math.floor(x0/step)*step; gx <= x1; gx += step) { c.moveTo(gx, y0); c.lineTo(gx, y1); }
			for (let gy = Math.floor(y0/step)*step; gy <= y1; gy += step) { c.moveTo(x0, gy); c.lineTo(x1, gy); }
			c.stroke();
		}
		for (let i = 0; i < items.length; i++) drawItem(c, items[i]);
	...
	function handleWheel(e) {
		...
		const z = Math.max(0.05, Math.min(8, viewport.zoom * factor));
	...
	function fitToContent() {
		...
		const pad = 48;
		const zoom = Math.max(0.05, Math.min(8, Math.min((W - pad * 2) / Math.max(1, b.width), (H - pad * 2) / Math.max(1, b.height))));
		const cx = b.x + b.width / 2, cy = b.y + b.height / 2;
		viewport = { x: W / 2 - cx * zoom, y: H / 2 - cy * zoom, zoom };
		redraw();
	}
	...
	function commit() {
		const kindMap = { draw: "freehand", arrow: "line", rect: "rect", ellipse: "ellipse", frame: "frame", text: "text" };
		const hostKind = kindMap[tool];
		const canCommit = tool === "text" ? curPoints.length >= 1 : curPoints.length >= 2;
		if (canCommit) {
			const pts = tool === "text" ? [curPoints[0]] : curPoints;
			items.push(localBuildItem(hostKind, color, size, pts, text, localId()));
			remote.addItem({ kind: hostKind, color, size, points: pts, text }).catch((err) => { console.error("addItem", err); });
		}
	...
	function onCanvasDown(e) {
		if (tool === "hand") { const n = e.nativeEvent || e; pan = { sx: n.offsetX, sy: n.offsetY, vx: viewport.x, vy: viewport.y }; return; }
		const w = screenToWorld(e);
		if (tool === "text") { curPoints = [w]; drawing = true; preview = { tool }; commit(); return; }
		drawing = true;
		curPoints = [w];
		preview = { tool };
		redraw();
	...
	function BoardPanel() {
		...
		const [canvasSize, setCanvasSize] = React.useState("large");
		...
		function pickTool(t) { tool = t; bump((n) => n + 1); redraw(); }
		function pickColor(c) { color = c; bump((n) => n + 1); }
		function toggleGrid() { gridOn = !gridOn; bump((n) => n + 1); redraw(); }
		function setSizeVal(v) { size = Math.max(1, Math.min(64, Number(v) || 4)); bump((n) => n + 1); }
		function setTextVal(v) { text = v; bump((n) => n + 1); }
		function clearBoard() { items = []; setCount(0); redraw(); remote.clear().catch((err) => { console.error("clear", err); }); }
		...
		const toolButtons = TOOLS.map((t) => React.createElement("button", { key: t.id, className: "cohub-tool" + (tool === t.id ? " on" : ""), title: t.label, onClick: () => pickTool(t.id) }, t.label));
		const swatches = COLORS.map((c) => React.createElement("button", { key: c.hex, className: "cohub-board-swatch" + (color === c.hex ? " on" : ""), style: { backgroundColor: c.hex }, onClick: () => pickColor(c.hex) }));
		...
		return React.createElement("div", {
			...
			React.createElement("div", { className: "cohub-toolbar" },
				toolButtons,
				swatches,
				React.createElement("input", { className: "cohub-size", type: "range", min: 1, max: 32, value: size, onChange: (e) => setSizeVal(e.target.value) }),
				...
				tool === "text" ? React.createElement("input", { className: "cohub-text", value: text, onChange: (e) => setTextVal(e.target.value) }) : null
			),
